...
export function createSessionWsHandler(
	...
	const handleUpgrade: UpgradeHandler = async (
		...
		wss.handleUpgrade(req, socket, head, (ws) => {
			wss.emit("connection", ws, req, {
				breezeSessionId: match.sessionId,
				auth,
	...
	wss.on(
		...
		async (ws: WebSocket, _req: IncomingMessage, context: SessionWsConnectionContext) => {
			const { breezeSessionId, auth } = context;

			try {
				logger.info({ sessionId: breezeSessionId, userId: auth.userId }, "Client connected");
				...
				ws.on("close", (code, reason) => {
					logger.info(
						{ sessionId: breezeSessionId, code, reason: reason?.toString() },
						"Client disconnected",
					);
				});
