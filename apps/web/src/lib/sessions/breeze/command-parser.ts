/**
 * Parser for `breeze` CLI bash command strings.
 *
 * Produces a typed discriminated union for the stable CLI surface:
 *   breeze actions list
 *   breeze actions guide --integration <id>
 *   breeze actions run --integration <id> --action <action> [--params <json>]
 *   breeze services <subcommand> [--name <n>]
 *   breeze env <subcommand>
 */

export type BreezeCommand =
	| { kind: "actions-list" }
...
/**
 * Parse a bash command string into a typed BreezeCommand.
 * Returns null if the command is not a `breeze` CLI invocation.
 */
export function parseBreezeCommand(command: string): BreezeCommand | null {
	const tokens = tokenize(command.trim());

	// Find the `breeze` token — may be preceded by env vars or a path
	const idx = tokens.findIndex((t) => t === "breeze" || t.endsWith("/breeze"));
