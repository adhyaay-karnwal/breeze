/**
 * Breeze CLI — env apply/scrub tests.
 *
...
function envScrub(
	...
	if (existsSync(breezeEnvFile)) {
		unlinkSync(breezeEnvFile);
		scrubbed.push(breezeEnvFile);
	}
...
describe("envApply", () => {
	...
	beforeEach(() => {
		workspaceDir = join(
			tmpdir(),
			`breeze-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
...
describe("envScrub", () => {
	...
	beforeEach(() => {
		...
		mkdirSync(workspaceDir, { recursive: true });
		breezeEnvFile = join(workspaceDir, ".breeze_env.json");
	});
	...
	it("removes secret-mode env files", () => {
		...
		const result = envScrub(spec, workspaceDir, breezeEnvFile);
	...
	it("removes the breeze env override file", () => {
		writeFileSync(breezeEnvFile, '{"KEY":"val"}');

		const result = envScrub([], workspaceDir, breezeEnvFile);
		expect(result.scrubbed).toContain(breezeEnvFile);
		expect(existsSync(breezeEnvFile)).toBe(false);
	});
	...
	it("rejects traversal paths in scrub spec", () => {
		...
		expect(() => envScrub(spec, workspaceDir, breezeEnvFile)).toThrow(/Path must not contain '..'/);
	});
});
