...
let setupBreezeWebSocket: Awaited<typeof import("./index")>["setupBreezeWebSocket"];
beforeAll(async () => {
	const mod = await import("./index");
	setupBreezeWebSocket = mod.setupBreezeWebSocket;
});
...
describe("WS handler message ordering", () => {
	...
	beforeEach(async () => {
		...
		setupBreezeWebSocket(server, mockHubManager as never, {} as never);
	...
	it("sends status resuming immediately after connection", async () => {
		const ws = new WebSocket(`ws://127.0.0.1:${port}/breeze/test-session-id?token=test-token`);
		openClients.push(ws);
	...
	it("rejects unauthenticated connections", async () => {
		...
		const ws = new WebSocket(`ws://127.0.0.1:${port}/breeze/test-session-id?token=bad-token`);
		openClients.push(ws);
