...
import { zodToJsonSchema } from "@breeze/providers/helpers/schema";
import { getProviderActions } from "@breeze/providers/providers/registry";
import { actions, sessions, userActionPreferences } from "@breeze/services";
...
export function createActionsRoutes(hubManager: HubManager): RouterType {
	...
	router.use((req, _res, next) => {
		req.breezeSessionId = req.params.breezeSessionId;
		next();
	});
	...
	router.get("/available", async (req, res, next) => {
		try {
			logger.debug({
				...
				route: "/available",
				params: req.params,
				breezeSessionId: req.breezeSessionId,
			});

			const sessionId = req.breezeSessionId!;
	...
	router.post("/invocations/:invocationId/approve", async (req, res, next) => {
		try {
			...
			const { invocationId } = req.params;
			const session = await requireSessionOrgAccess(req.breezeSessionId!, auth.orgId);
			const targetInvocation = await actions.getActionStatus(invocationId, session.organizationId);
			if (!targetInvocation || targetInvocation.sessionId !== req.breezeSessionId) {
				throw new ApiError(404, "Invocation not found");
			}
