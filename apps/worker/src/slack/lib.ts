 ...
 * Only show verification results - other tools are too noisy
 */
export function shouldPostTool(toolName: string): boolean {
	const significantTools = ["verify"];
	return significantTools.some(
		(t) => toolName.toLowerCase().includes(t) || toolName.toLowerCase() === t,
	);
}
...
/**
 * Post verification results with media previews and dashboard link
 */
export async function postVerificationResult(
	...
	try {
		...
		const { files } = (await listResponse.json()) as {
			files: Array<{ key: string; name: string; path: string; contentType: string; size: number }>;
		};
		...
		// 2. Separate media files from others
		...
		// 3. Upload media files to Slack (limit to first 5 to avoid spam)
		const filesToUpload = mediaFiles.slice(0, 5);
		for (const file of filesToUpload) {
			try {
				// Stream the file from our API (handles S3 auth)
				const fileResponse = await fetch(
					`${apiUrl}/api/verification-media?key=${encodeURIComponent(file.key)}&stream=true`,
				);
		...
		// 4. Post summary with dashboard link
		const orgQuery = orgId ? `?orgId=${orgId}` : "";
		const sessionUrl = `${appUrl}/dashboard/sessions/${sessionId}${orgQuery}`;
		...
		if (otherFiles.length > 0) {
			summaryParts.push(
				`${otherFiles.length} other file(s): ${otherFiles.map((f) => f.name).join(", ")}`,
			);
		}
