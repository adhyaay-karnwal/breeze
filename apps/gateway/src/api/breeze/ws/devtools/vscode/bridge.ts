...
const logger = createLogger({ service: "gateway" }).child({ module: "vscode-ws" });

export async function bridgeVscodeWebSocket(
	req: IncomingMessage,
	socket: Duplex,
	head: Buffer,
	sessionId: string,
	...
	const upstreamPath = `/_breeze/vscode${tailPath || "/"}${search}`;
	const upstreamUrl = `${previewUrl.replace(/^http/, "ws")}${upstreamPath}`;

	logger.info({ sessionId }, "Proxying VS Code WS");
	...
	upstream.on("error", (error) => {
		logger.error({ err: error, sessionId }, "VS Code upstream WS error");
		removeProxy();
