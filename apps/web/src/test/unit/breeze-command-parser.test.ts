import {
	...
	parseBreezeCommand,
	resolveIconKey,
} from "@/lib/sessions/breeze/command-parser";
import { describe, expect, it } from "vitest";

describe("parseBreezeCommand", () => {
	describe("non-breeze commands", () => {
		it("returns null for empty string", () => {
			expect(parseBreezeCommand("")).toBeNull();
		});

		it("returns null for unrelated commands", () => {
			expect(parseBreezeCommand("ls -la")).toBeNull();
			expect(parseBreezeCommand("git status")).toBeNull();
			expect(parseBreezeCommand("echo hello")).toBeNull();
		});

		it("returns null for breeze without subcommand", () => {
			expect(parseBreezeCommand("breeze")).toBeNull();
		});

		it("returns null for unknown group", () => {
			expect(parseBreezeCommand("breeze unknown command")).toBeNull();
		});
	});

	describe("actions list", () => {
		it("parses bare actions list", () => {
			expect(parseBreezeCommand("breeze actions list")).toEqual({
				kind: "actions-list",
			});
		});
		...
		it("parses with shell redirect suffix", () => {
			expect(
				parseBreezeCommand("breeze actions list 2>/dev/null || echo 'No integrations'"),
	...
	describe("actions guide", () => {
		it("parses integration flag", () => {
			expect(
				parseBreezeCommand(
					'breeze actions guide --integration "connector:701f8f1c-0f8d-4b2c-a983-25c8f24d2881"',
				),
			).toEqual({
				kind: "actions-guide",
		...
		it("returns null when integration is missing", () => {
			expect(parseBreezeCommand("breeze actions guide")).toBeNull();
		});
	});
	...
	describe("actions run", () => {
		it("parses minimal run command", () => {
			expect(
				parseBreezeCommand(
					'breeze actions run --integration "connector:abc" --action GMAIL_FETCH_EMAILS',
				),
			).toEqual({
				kind: "actions-run",
		...
		it("parses run with JSON params (single-quoted)", () => {
			const result = parseBreezeCommand(
				`breeze actions run --integration "connector:abc" --action GMAIL_FETCH_EMAILS --params '{"max_results": 10, "verbose": true}'`,
			);
		...
		it("parses run with double-quoted params", () => {
			const result = parseBreezeCommand(
				`breeze actions run --integration "connector:abc" --action GMAIL_FETCH_MESSAGE_BY_MESSAGE_ID --params "{\\"message_id\\": \\"abc123\\"}"`,
			);
			expect(result).toEqual({
				kind: "actions-run",
				integration: "connector:abc",
		...
		it("tolerates invalid JSON in params and sets params to null", () => {
			const result = parseBreezeCommand(
				"breeze actions run --integration linear --action list_teams --params 'not-json'",
			);
			expect(result).toEqual({
				kind: "actions-run",
				integration: "linear",
		...
		it("parses native integration (no connector prefix)", () => {
			const result = parseBreezeCommand(
				"breeze actions run --integration linear --action list_issues",
			);
			expect(result).toEqual({
				kind: "actions-run",
				integration: "linear",
		...
		it("returns null when integration is missing", () => {
			expect(parseBreezeCommand("breeze actions run --action GMAIL_FETCH_EMAILS")).toBeNull();
		});

		it("returns null when action is missing", () => {
			expect(parseBreezeCommand('breeze actions run --integration "connector:abc"')).toBeNull();
		});

		it("ignores shell suffix after operator", () => {
			const result = parseBreezeCommand(
				`breeze actions run --integration "connector:abc" --action GMAIL_LIST_MESSAGES --params '{"max_results": 5}' 2>&1`,
			);
			expect(result).toEqual({
				kind: "actions-run",
				integration: "connector:abc",
	...
	describe("services", () => {
		it("parses services list", () => {
			expect(parseBreezeCommand("breeze services list")).toEqual({
				kind: "services",
				subcommand: "list",
				name: undefined,
			});
		...
		it("parses services stop with name", () => {
			expect(parseBreezeCommand("breeze services stop --name my-server")).toEqual({
				kind: "services",
				subcommand: "stop",
				name: "my-server",
			});
	...
	describe("env", () => {
		it("parses env apply", () => {
			expect(parseBreezeCommand("breeze env apply --spec '{}'")).toEqual({
				kind: "env",
				subcommand: "apply",
			});
		});
