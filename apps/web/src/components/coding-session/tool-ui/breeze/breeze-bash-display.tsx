...
import { normalizeActionsCatalog } from "@/lib/sessions/breeze/catalog";
import type { BreezeCommand } from "@/lib/sessions/breeze/command-parser";
import { resolveIconKey } from "@/lib/sessions/breeze/command-parser";
import {
	normalizeGmailMessageDetail,
	normalizeGmailMessageHandles,
	normalizeGmailMessageList,
	normalizeGmailMutationSummary,
} from "@/lib/sessions/breeze/gmail";
import {
	humanizeAction,
	resolveIntegrationLabel,
	resolvePresentation,
} from "@/lib/sessions/breeze/presentation-registry";
...
interface BreezeBashDisplayProps {
	parsed: BreezeCommand;
	result?: unknown;
	status?: { type: string };
	command?: string;
}

export function BreezeBashDisplay({ parsed, result, status, command }: BreezeBashDisplayProps) {
	...
	if (parsed.kind === "actions-guide") {
		...
		return (
			<ActionCardShell
				...
				status={status}
				meta={`${integrationLabel} via Breeze CLI`}
				command={command}
				rawResult={result}
				errorMessage={errorMessage}
			>
