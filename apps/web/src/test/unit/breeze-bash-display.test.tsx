	...
	it("renders guide output instead of dropping it", () => {
		const html = renderToStaticMarkup(
			<BreezeBashDisplay
				...
				result={"# Guide\nUse list_issues"}
				status={{ type: "done" }}
				command={'breeze actions guide --integration "linear"'}
			/>,
		);
		...
		expect(html).not.toContain("breeze actions guide --integration &quot;linear&quot;");
	});
		...
		const html = renderToStaticMarkup(
			<BreezeBashDisplay
				parsed={{
					...
					params: { recipient_email: "team@breeze.engineer" },
				}}
