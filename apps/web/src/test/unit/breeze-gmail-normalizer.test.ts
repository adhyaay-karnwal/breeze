...
describe("normalizeGmailMessageDetail", () => {
	const detailResponse = {
		data: {
			...
			payload: {
				headers: [
					{ name: "From", value: "Dabl Club <claw@mail.beehiiv.com>" },
					{ name: "To", value: "adhyaay@breeze.engineer" },
					{
						name: "Subject",
	...
	it("handles draft detail results with nested message payload", () => {
		const draftResponse = {
			data: {
				...
				message: {
					...
					payload: {
						headers: [
							{ name: "From", value: "Pablo <adhyaay@breeze.engineer>" },
							{ name: "To", value: "team@breeze.engineer" },
		...
		expect(detail).toEqual({
			id: "draft-123",
			from: "Pablo <adhyaay@breeze.engineer>",
			to: "team@breeze.engineer",
