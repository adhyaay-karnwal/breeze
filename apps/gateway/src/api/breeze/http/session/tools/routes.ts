...
export function createToolsRoutes(hubManager: HubManager): Router {
	...
	router.post("/:toolName", async (req, res, next) => {
		try {
			const { breezeSessionId, toolName } = req.params as Record<string, string>;
			const { tool_call_id: toolCallId, args } = req.body as {
				tool_call_id?: string;
				args?: Record<string, unknown>;
			};

			if (!breezeSessionId) throw new ApiError(400, "Missing session ID");
			...
			const key = cacheKey(breezeSessionId, toolName, toolCallId);
